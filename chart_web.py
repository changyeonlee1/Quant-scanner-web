...
def analyze_single_stock(code, name, ticker_yf, condition_type, start_date, use_shield):
    try:
        ...
        curr_low = float(df['Low'].iloc[-1]) # 현재 캔들 저가 (FVG 확인용)
        ...
        # 🌟 FVG (공정 가치 갭) 탐지 로직 (최근 15일 기준)
        fvg_top = None
        fvg_bottom = None
        if condition_type in ["F", "ALL"]:
            for i in range(len(df)-15, len(df)-2):
                c1_high = float(df['High'].iloc[i])
                c3_low = float(df['Low'].iloc[i+2])
                
                # 상승 FVG 발견 (3번 캔들 저가가 1번 캔들 고가보다 높음)
                if c3_low > c1_high:
                    is_filled = False
                    # 그 이후 캔들이 FVG 하단까지 완전히 침범해서 갭을 메웠는지 검사
                    for j in range(i+3, len(df)-1):
                        if float(df['Low'].iloc[j]) <= c1_high:
                            is_filled = True
                            break
                    
                    # 아직 안 메워진 '살아있는 FVG'라면 저장 (가장 최근 것으로 덮어씀)
                    if not is_filled:
                        fvg_top = c3_low
                        fvg_bottom = c1_high
        ...
        if condition_type in ["F", "ALL"]:
            if fvg_top is not None and fvg_bottom is not None:
                # 오늘 주가가 FVG 구역 내부로 들어왔고 과매수 상태가 아닐 때
                if fvg_bottom <= curr_low <= fvg_top and curr_rsi < 55:
                    matched_conds.append("F")
                    reasons.append(f"[🏦F] FVG진입({fvg_bottom:,.0f}~{fvg_top:,.0f})")
...
with st.sidebar:
    ...
    strategy_options = {
        ...
        "🍚 E. 밥그릇 3번 자리": "E", "🏦 F. 스마트 머니 (FVG 되돌림)": "F", 
        "🌟 전술 통합 스캔 (ALL)": "ALL"
    }
